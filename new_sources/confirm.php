<?php
	//NEW USER CONFIRM
		if (isset($_POST['ok']) && !isset($_POST['logged']) && empty($_SESSION['a_id'])) {	
			...
			EmailSender::sendAdminNotificationNewAuthor($_SESSION['author_id']);
...
?>
	...
	<div Id="content">
		<div style="text-align: center;">
				<i>&nbsp1. Registrácia účastníka ----> 2. Registrácia robota/robotov ----> </i><b>3. Potvrdenie, koniec</b>
				<br/><br/>
			</div>
		<h3>Do súťaže Istrobot <?php echo $_SESSION['year'];?> ste úspešne prihlásili robota/robotov:</h3>
		<?php
			//exist user confirm
			...
			while ($competition = mysql_fetch_object($sql)) {
				$cat_name[$i] = $competition->name;
				$cat_id[$i] = $competition->id;
				$i++;
			}
		// $i-poc kategorii, $j-premenna pre kazdu kategoriu, $k-premenna pre kazdeho robota danej kategorie	
			for ($j=1; $j<$i; $j++){
				...
				$sql = mysql_query("SELECT author.name_surname, author.state, author.town, robot.name, robot.subauthor FROM author INNER JOIN robot ON author.id_author = robot.id_author JOIN competitions_contestants ON robot.id_robot = competitions_contestants.robot_id WHERE competitions_contestants.competition_id = '".$cat_id[$j]."' AND robot.show = 1 AND author.id_author ='".$a_id."'") or die(mysql_error());
				$k=1;
		...
		if ($wim == 1){
			...
			echo "<input type=\"Submit\" name = \"edit\" value=\"Uprav\">";
			echo "</form>";
			echo "</div>";
		} elseif ($wim == 0)  {
			echo "<div style=\"text-align: right;\">";
		...
		<table style="border: 1px solid red;">
			<tr id="state"><td><b>Som oboznámený s <a href="#rules" class="show_hide">Pravidlam súťaže</a> a prihlasujem uvedeného robota (robotov) na súťaž Istrobot <?php echo $_SESSION['year'];?>.</b></td></tr>
		...
		<div id="rules" class="slidingDiv"> 
			...
			<br/>		
			<a href="#state" class="show_hide">Skryť</a>
		</div>
	</div>