 <?php
		...
		if (isset($_GET['key']) && empty($_SESSION['a_id'])) {
			...
			//zaslanie aktivacneho mailu po evidencii uzastinika, robotu sa nastavi atribut default show = 0 
				...
				if (!mysql_num_rows($sql_check_hash)) {
						$mail = $_SESSION['mail'];
						$pasw = $_POST['pasw'];
						mysql_query("insert into activation values ('',".$a_id.",'".activate_hash($mail, $pasw, $reg_time)."')") or die ("activation error");
	...
	////KED SOM PRIHLASENY VYPIS			
			...
			if ($_GET['action'] == "delete"){
				...
				mysql_query("UPDATE category_tag SET year = 0 WHERE id_robot = '".$pieces[0]."'") or die("Error fadeing CATEGORY_TAG: ".mysql_error()."");
			...
			//novy zapis dat z DB do premennych
				...
				$sql2 = mysql_query("select id_category from category_tag where id_robot ='".$robot_id."'");

				
				$ch=0;
				while ($category = mysql_fetch_object($sql2)) {
					$checkbox[$ch] = $category->id_category;
					$ch++; 
				}
				$errmsg = "";
...
?>
		...
		<div Id="content">
			...
			<P>
			<UL>
				 <LI> One application is valid also for more categories.</li>
				 <LI> You can register with more than one robot into the category, but it is necessary to fill one application for each robot. </li>
				 <LI> Contest is open to any individuals or teams (max. 5 members) without any age or occupation limits.</li>
				 <LI> The application deadline is <B>1.&nbsp;april 2013</B></li>
				 <LI> <B>Use</B> diacritics, please!</li>
			</UL>
			...
			<?php
				if ($wim == 0){
					...
					echo "<H2>Personal profile of participant</H2>";
				}				
			?>
			...
			<script type="text/javascript">
				...
				{
				  if(document.pressed == 'Edit robots')
				  {
				   document.myform.action ="index.php?page=lor";
				  } else if(document.pressed == 'Edit' || document.pressed == 'Continue registration of robot')
				  {
			...
			// vypis mena robota/ zoznamu robotov pre prihlaseneho uzivatela	
				if ($wim == 0){
					...
					echo "<TR><TD></TD><TD class=\"title\">List of robots:</TD>";
					...
					echo "<TD></TD><TD class=\"title\"></TD><TD class=\"input\"><INPUT Type=\"submit\"  Name=\"edit\" onclick=\"document.pressed=this.value\" value=\"Edit robots\"></TD></TR>";
					//echo "</select></TD>";
			...
			<TD class="input"><INPUT Type="text" Name="job" Size="30" value="<?php echo $job; ?>"></TD><TD></TD></TR>
				<?php
					...
					if ($wim == 2){
						echo "<TR><TD class=\"reddot\">●</TD><TD class=\"title\">Competition category:</TD>";
						echo "<TD class=\"input\">";
						echo "<UL>";
						openMySQL($host, $user, $passwd, $db);
						...
						echo "<TD class=\"notice\"> ● Required entry</TD>";
				...
				?>
			<TR><TD class="reddot"></TD><TD class="title">Contact (address, tel.):      </TD>
			<TD class="input"><TEXTAREA  Name="address" Rows="3" Cols="24"><?php echo $errmsg; ?></TEXTAREA></TD><TD class="notice"></TD></TR>
				...
				?>
			<TR><TD class="reddot"></TD><TD class="title">Town:     </TD>
			<TD class="input"><Input Type="text" Name="town" id = "town" Size="30" value="<?php echo $errmsg; ?>"></TD><TD class="notice"> ● <?php echo $errmsg2; ?></TD></TR>	
				...
				?>
			<TR><TD class="reddot"></TD><TD class="title">Country:     </TD>
			<TD class="input">
				<select name= "state" style="width:217px">
					<?php
						$state_name = array("Slovakia", "Czech Republic", "Poland", "Austria", "Germany", "other...");
			...
			<TD class="input"><Input Type="text" Name="mail" Size="30" disabled="disabled" value="<?php echo $errmsg; ?>"></TD><TD class="notice"> ● </TD></TR>
				<?php
						if (($key[6] == 0) && (isset($_GET['key']))) {
							$errmsg = " Passwords do not match or too short (min. 3 chars)!";
						} else {
							$errmsg = "";
						}
			...
			<?php 
				...
				//CAPTCHA VYPIS
					...
					echo "<TR><TD class=\"reddot\"></TD><TD class=\"title\">Which text is here?</TD>";
				...
				if ($wim == 0){
					echo "<TABLE>";
					echo "<TR><TD class=\"reddot\"></TD><TD class=\"title\"></TD><TD class=\"input\"><INPUT Type=\"submit\" onclick=\"document.pressed=this.value\" Value=\"Continue registration of robot\"></TD></TR>";
					echo "</TABLE>";
				} else {
					echo "<br/>";
